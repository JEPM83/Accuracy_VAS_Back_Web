...
app.MapPost("/accuracy/vas/api/v1/GetCorrelativeLpnDestiny",
    ...
    .WithTags("Vas");
//
app.MapPost("/accuracy/vas/api/v1/GetB2BAdvance",
    [AllowAnonymous] async ([FromBody] B2BAdvanceRequest obj, HttpContext context) =>
    {
        var authorizationHeader = context.Request.Headers["Authorization"].FirstOrDefault();
        ...
        if (!StringValues.IsNullOrEmpty(authorizationHeader) && authorizationHeader.StartsWith("Bearer "))
        {
            ...
            try
            {
                ...
                List<B2BAdvanceResponse> resp = poBL.GET_B2B_ADVANCE(obj, connString);
                if (resp == null || resp.Count == 0)
                {
                    var errorResponse = new
                    {
    ...
    .Accepts<B2BAdvanceRequest>("application/json")
    .Produces<List<B2BAdvanceResponse>>(StatusCodes.Status200OK)
    .WithName("GetB2BAdvance")
    .WithTags("Vas");
//
#endregion
app.Run();
